...
try:
    cursor.execute('INSERT INTO Software (Title, Version, Developer, Description) VALUES (?, ?, ?, ?)',
                   (title, version, developer, description))
    con.commit()
except Exception as e:
    print(f"<div>Oshibka pri oformlenii abonimenta: {e}</div>")
else:
    print("<div>Aboniment uspeshno dobavlen!</div>")
finally:
    con.close()
